    ...
    max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'} `}>
        <div>
            <div className={`flex  ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
                ...
                <div onClick={()=> expand ? setExpand(false) : setExpand(true)} className='group relative flex items-center justify-center
                 hover:bg-gray-500/20 transition-all duration-300 h-9 w-9 aspect-square rounded-lg cursor-pointer'>
                    ...
                    <div className={`absolute w-max ${expand ? "left-1/2 -translate-x-1/2 -top-12" : "-top-12 left-0"} opacity-0 group-hover:opacity-100 transition-opacity bg-black text-white text-sm px-3 py-2 rounded-lg
           ...
           <button className={`mt-8 flex items-center justify-center cursor-pointer 
            ${expand ? "bg-primary hover: opacity-90 rounded-2xl gap-2 p-2.5 w-max": "group relative h-9 w-9 mx-auto hover:bg-gray-500/20 rounded-lg transition-all duration-300"}`}>
